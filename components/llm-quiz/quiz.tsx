        ...
        <div className="space-y-2">
                ...
                <div className="w-1/5 text-right">
                  {isAnswerCorrect == true && answer == option.id ? (
                    <div className="flex flex-row items-center text-xs leading-none text-green-500 gap-2 p-4">
                      <CheckIcon /> <p className="hidden sm:block">Correct</p>
                    </div>
                  ) : (
                  ...
                  {isAnswerCorrect == false && selectedOption == option.id ? (
                    <div className="flex flex-row items-center text-xs leading-none text-red-500 gap-2">
                      <WrongIcon /> <p className="hidden sm:block">Incorrect</p>
                    </div>
                  ) : (
                    <div className="flex flex-row items-center text-xs leading-none text-green-500">
                      <p></p>
          ...
          {isAnswerCorrect == true ? (
            <Button
              ...
              onClick={async () => {
                const response =
                  await submitUserMessage(`Give me another test`);
          ...
          {isAnswerCorrect == false ? (
            <Button
              className="mt-2 p-4 bg-[#EE702C] text-md"
              onClick={async () => {
                const response =
                  await submitUserMessage(`Give me another test`);
                setMessages((currentMessages) => [
